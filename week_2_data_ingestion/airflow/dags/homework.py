...
class Base:
    ...
    def build_gcp_path(self):
        pass
    
    def build_dataset_url(self):
        pass
    ...
    def run_dag(self):
        with DAG:
            dataset_url = self.build_dataset_url('execution_date.strftime(\'%Y\')', 'execution_date.strftime(\'%m\')')
            download_dataset_task = BashOperator(
                task_id="download_dataset_task",
                bash_command=f"curl -sSL {dataset_url} > {self.build_local_path()}"
            )
            ...
            local_to_gcs_task = PythonOperator(
                ...
                op_kwargs={
                    "bucket": BUCKET,
                    "object_name": self.build_gcp_path(),
                    "local_file": self.build_local_path(),
                },
            )
