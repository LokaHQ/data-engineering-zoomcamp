...
with DAG(
    ...
    for colour, ds_col in COLOUR_RANGE.items():
        ...
        bigquery_external_table_task = BigQueryCreateExternalTableOperator(
            ...
            table_resource={
                ...
                "externalDataConfiguration": {
                    "autodetect": "True",
                    "sourceFormat": INPUT_FILETYPE.upper(),
        ...
        EXCEPT = ""
        if colour == 'green':
            EXCEPT = " EXCEPT (`ehail_fee`) "
        if colour == 'yellow':
            EXCEPT = " EXCEPT (`airport_fee`) "
        if colour == "fhv":
            EXCEPT = " EXCEPT (`SR_Flag`, `DOlocationID`, `PUlocationID`) "

        CREATE_BQ_TBL_QUERY = (
            f"CREATE OR REPLACE TABLE {BIGQUERY_DATASET}.{colour}_{DATASET} \
            PARTITION BY DATE({ds_col}) \
            AS \
            SELECT * {EXCEPT} FROM {BIGQUERY_DATASET}.{colour}_{DATASET}_external_table;"
        )
