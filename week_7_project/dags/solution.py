...
KNOWN_REGIONS = ["BR", "CA", "DE", "FR", "GB", "IN", "JP", "KR", "MX", "RU", "US"]
...
def transfer_data_from_kaggle_to_gcs(
    ...
    with dag:
        download_dataset_task = BashOperator(
            ...
            bash_command=f"kaggle datasets download -d fronkongames/steam-games-dataset -p /opt/airflow/data"
        )
        unzip_dataset_task = BashOperator(
            task_id="unzip_dataset_task",
            bash_command=f"unzip /opt/airflow/data/steam-games-dataset.zip -d /opt/airflow/data"
        )
...
# YELLOW_TAXI DAG
YOUTUBE_PREFIX_FOLDER = AIRFLOW_HOME + \
    '/data/'
YOUTUBE_PREFIX_GCS_FOLDER = "raw/steam_games_dataset/"
YOUTUBE_CSV_SUFFIX = 'games.csv'
YOUTUBE_PARQUET_SUFFIX = 'games.parquet'

yellow_taxi_data_dag = DAG(
    dag_id="yellow_taxi_v16",
...
transfer_data_from_kaggle_to_gcs(
    ...
    prefixFolder=YOUTUBE_PREFIX_FOLDER,
    prefixGCSFolder=YOUTUBE_PREFIX_GCS_FOLDER,
    suffixCsv=YOUTUBE_CSV_SUFFIX,
    suffixParquet=YOUTUBE_PARQUET_SUFFIX,
)
